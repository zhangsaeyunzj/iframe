...
public class FileAction implements Action {
    ...
    public void doAction(Request request, Response response) {
        ...
        String ifModifiedSince = request
                .getHeader(HttpHeaderNames.IF_MODIFIED_SINCE.toString());
        if (StrUtil.isNotBlank(ifModifiedSince)) {
            Date ifModifiedSinceDate = null;
            try {
                SimpleDateFormat sdf = new SimpleDateFormat(DateUtil.HTTP_DATETIME_PATTERN, Locale.US);
                ifModifiedSinceDate = DateUtil.parse(ifModifiedSince, sdf);
            } catch (Exception e) {
                log.warn("If-Modified-Since header parse error: {}",
                        e.getMessage());
            }
    ...
    public static File getFileByPath(String httpPath) {
        ...
        // 路径安全检查
        if (httpPath.contains("/.") || httpPath.contains("./")
                || httpPath.charAt(0) == '.'
                || httpPath.charAt(httpPath.length() - 1) == '.'
                || ReUtil.isMatch(INSECURE_URI, httpPath)) {
            return null;
        }
