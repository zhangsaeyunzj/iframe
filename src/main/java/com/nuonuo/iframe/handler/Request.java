...
public class Request {
	...
	private Request(ChannelHandlerContext ctx, FullHttpRequest nettyRequest) {
		...
		if (nettyRequest.method() != HttpMethod.GET) {
			...
			try {
				decoder = new HttpPostRequestDecoder(HTTP_DATA_FACTORY,
						nettyRequest);
	...
	public boolean isKeepAlive() {
		final String connectionHeader = getHeader(HttpHeaderNames.CONNECTION
				.toString());
		// 无论任何版本Connection为close时都关闭连接
		if (HttpHeaderValues.CLOSE.toString()
		...
		if (HttpVersion.HTTP_1_0.text().equals(getProtocolVersion())) {
			if (false == HttpHeaderValues.KEEP_ALIVE.toString()
					.equalsIgnoreCase(connectionHeader)) {
				return false;
			}
		}
	...
	protected void putParams(QueryStringDecoder decoder) {
		if (null != decoder) {
			...
			for (Entry<String, List<String>> entry : decoder.parameters()
					.entrySet()) {
				valueList = entry.getValue();
				if (null != valueList) {
					this.putParam(entry.getKey(),
							1 == valueList.size() ? valueList.get(0)
									: valueList);
				}
			}
		}
	...
	protected void putHeadersAndCookies(HttpHeaders headers) {
		...
		if (StrUtil.isNotBlank(cookieString)) {
			final Set<Cookie> cookies = ServerCookieDecoder.LAX
					.decode(cookieString);
	...
	protected void putIp(ChannelHandlerContext ctx) {
		...
		if (StrUtil.isNotBlank(ip)) {
			...
			final InetSocketAddress insocket = (InetSocketAddress) ctx
	...
	public String toString() {
		...
		sb.append("\r\nprotocolVersion: ").append(getProtocolVersion())
				.append("\r\n");
	 ...
	 * @return Request
	 */
	protected final static Request build(ChannelHandlerContext ctx,
