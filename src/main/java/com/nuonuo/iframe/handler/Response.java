...
import com.nuonuo.iframe.listener.FileProgressiveFutureListener;
...
public class Response {
    ...
    public Response setKeepAlive() {
        setHeader(HttpHeaderNames.CONNECTION.toString(),
    ...
    public Response setContent(String contentText) {
        this.content = Unpooled.copiedBuffer(contentText,
    ...
    public void setDateAndCache(long lastModify, int httpCacheSeconds) {
        SimpleDateFormat formatter = new SimpleDateFormat(
                DateUtil.HTTP_DATETIME_PATTERN, Locale.US);
        formatter.setTimeZone(TimeZone.getTimeZone("GMT"));

        // Date header
        Calendar time = new GregorianCalendar();
        setHeader(HttpHeaderNames.DATE.toString(),
                formatter.format(time.getTime()));

        // Add cache headers
        time.add(Calendar.SECOND, httpCacheSeconds);

        setHeader(HttpHeaderNames.EXPIRES.toString(),
                formatter.format(time.getTime()));
        setHeader(HttpHeaderNames.CACHE_CONTROL.toString(), "private, max-age="
                + httpCacheSeconds);
        setHeader(HttpHeaderNames.LAST_MODIFIED.toString(),
                formatter.format(DateUtil.date(lastModify)));
    }
    ...
    private DefaultHttpResponse toDefaultHttpResponse() {
        ...
        // headers
        final HttpHeaders httpHeaders = defaultHttpResponse.headers().add(
                headers);
    ...
    private FullHttpResponse toFullHttpResponse() {
        ...
        final FullHttpResponse fullHttpResponse = new DefaultFullHttpResponse(
                httpVersion, status, byteBuf);

        // headers
        final HttpHeaders httpHeaders = fullHttpResponse.headers().add(headers);
        httpHeaders.set(HttpHeaderNames.CONTENT_TYPE.toString(),
                StrUtil.format("{};charset={}", contentType, charset));
        //httpHeaders.set(HttpHeaderNames.CONTENT_ENCODING.toString(), charset);
        httpHeaders.set(HttpHeaderNames.CONTENT_LENGTH.toString(),
    ...
    private ChannelFuture sendAndCloseFull() {
        return ctx.writeAndFlush(this.toFullHttpResponse()).addListener(
    ...
    private ChannelFuture sendFile(File file) throws IOException {
        ...
        ctx.write(new DefaultFileRegion(raf.getChannel(), 0, fileLength),
                ...
                FileProgressiveFutureListener.build(raf));
    ...
    private ChannelFuture sendEmptyLast() {
        final ChannelFuture lastContentFuture = ctx
    ...
    public ChannelFuture sendRedirect(String uri) {
        return this.setStatus(HttpResponseStatus.FOUND)
                .setHeader(HttpHeaderNames.LOCATION.toString(), uri).send();
    }
    ...
    public ChannelFuture sendNotModified() {
        return this
                ...
                .setHeader(HttpHeaderNames.DATE.toString(),
                        DateUtil.formatHttpDate(DateUtil.date())).send();
    }
